    ...
    <body class="bg-gray-50 min-h-screen" x-data="packingApp()" x-init="init()">
        ...
        <div x-show="!volunteerName" x-transition class="fixed inset-0 bg-gray-900/95 z-60 flex items-center justify-center px-4">
            <div class="bg-white rounded-2xl p-6 max-w-sm w-full text-center shadow-xl">
                ...
                <input type="text" x-ref="volunteerNameInput" autofocus
                    @keydown.enter="saveVolunteerName($refs.volunteerNameInput.value)"
                    class="w-full text-center text-lg py-3 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-blue-500 mb-3"
                    placeholder="Your name">
                @if(\App\Models\Setting::get('google_client_id'))
                <div class="text-xs text-gray-400 mb-3">or</div>
                <a href="{{ url('/auth/google') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition">
                ...
                @endif
                <button @click="saveVolunteerName($refs.volunteerNameInput.value)"
                    class="mt-3 w-full py-3 bg-green-600 text-white rounded-lg font-semibold text-sm hover:bg-green-500 transition">
                    Start Packing
                </button>
            </div>
        </div>
        ...
        <div class="bg-white border-b sticky top-0 z-40 px-4 py-3">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-xs text-gray-500">{{ $packingList->items->count() }} items to pack
                        <span x-show="volunteerName" class="text-blue-600" x-text="'&middot; ' + volunteerName"></span>
                    </p>
                </div>
        ...
        function packingApp() {
            return {
                ...
                audioCtx: null,
                volunteerName: @json(auth()->user()?->first_name) || localStorage.getItem('packing_volunteer_name') || '',

                saveVolunteerName(name) {
                    name = (name || '').trim();
                    if (!name) return;
                ...
                async scanBarcode(barcode) {
                    ...
                    try {
                        const res = await fetch(`/api/packing/${this.listId}/scan`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
                            body: JSON.stringify({ barcode: barcode.trim(), volunteer_name: this.volunteerName }),
                        });
                        const data = await res.json();
