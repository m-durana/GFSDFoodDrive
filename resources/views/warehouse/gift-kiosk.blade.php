...
<head>
    ...
    <title>Gift Kiosk - GFSD Food Drive</title>
...
<body class="font-sans antialiased bg-gray-900 text-gray-100 min-h-screen flex flex-col">
    ...
    <div class="bg-gray-800 border-b border-gray-700 px-6 py-4 flex justify-between items-center">
        <h1 class="text-xl font-bold text-purple-400">Gift Kiosk</h1>
    ...
    <div id="volunteer-prompt" class="fixed inset-0 bg-gray-900/95 z-50 flex items-center justify-center" style="display:none;">
        <div class="bg-gray-800 rounded-xl p-8 max-w-md w-full border border-gray-700 text-center">
            ...
            <input type="text" id="volunteer-name-input" autofocus
                class="w-full text-center text-lg py-3 rounded-lg bg-gray-700 border border-gray-600 text-gray-100 placeholder-gray-500 focus:border-purple-500 focus:ring-purple-500"
                placeholder="Your name">
            <button onclick="saveVolunteerName()" class="mt-4 w-full py-3 bg-purple-600 text-white rounded-lg font-medium hover:bg-purple-500 transition">
                Start Scanning
            </button>
        </div>
    </div>
        ...
        <div class="flex-1 flex flex-col items-center justify-start p-8 space-y-6 overflow-y-auto">
            ...
            <div class="w-full max-w-lg">
                <input type="text" id="kiosk-barcode" autofocus autocomplete="off"
                    class="w-full text-center text-3xl py-6 rounded-xl bg-gray-800 border-2 border-gray-600 text-gray-100 placeholder-gray-500 focus:border-purple-500 focus:ring-purple-500 transition"
                    placeholder="Scan or type barcode...">
                <p class="text-xs text-gray-600 mt-1 text-center">Press Enter to submit. Shortcut keys (a-z, 0-9) to quick-select gift categories.</p>
            </div>

            <!-- Loose Donation Form (shown after unrecognised barcode scan) -->
            <div id="loose-form" class="w-full max-w-lg hidden">
                <div class="bg-gray-800 border border-purple-700 rounded-xl p-5 space-y-3">
                    <h3 class="text-sm font-semibold text-purple-300">Unrecognised Item — Add Gift Details</h3>
                    <p class="text-xs text-gray-400" id="loose-barcode-label"></p>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs text-gray-500 mb-1">Age Suitability</label>
                            <select id="loose-age" class="w-full rounded-lg bg-gray-700 border border-gray-600 text-gray-100 text-sm py-1.5 px-3 focus:border-purple-500 focus:ring-purple-500">
                        ...
                        <div>
                            <label class="block text-xs text-gray-500 mb-1">Gender Suitability</label>
                            <select id="loose-gender" class="w-full rounded-lg bg-gray-700 border border-gray-600 text-gray-100 text-sm py-1.5 px-3 focus:border-purple-500 focus:ring-purple-500">
                    ...
                    <div class="flex gap-2">
                        <button onclick="submitLooseDonation()" class="flex-1 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-500 transition">
                            Add to Gift Bank
                        </button>
                        <button onclick="dismissLooseForm()" class="px-4 py-2 bg-gray-700 text-gray-400 rounded-lg text-sm hover:bg-gray-600 transition">
                            Skip
                        </button>
                    </div>
                </div>
                    ...
                    @foreach($categories as $i => $cat)
                        <button data-category-id="{{ $cat->id }}" data-category-name="{{ $cat->name }}" data-default-shortcut="{{ $defaultKeys[$i] ?? '' }}"
                            class="kiosk-cat-btn p-4 rounded-xl text-center transition font-medium text-sm relative
                                bg-purple-900/40 border border-purple-700 hover:bg-purple-900/60 text-purple-200">
        ...
        <div id="details-sidebar" class="w-72 flex-shrink-0 bg-gray-800 border-l border-gray-700 p-4 space-y-4 overflow-y-auto sticky top-0 h-screen">
            ...
            <div>
                ...
                <input type="number" id="detail-quantity" value="1" min="1" max="9999"
                    class="w-full rounded-lg bg-gray-700 border border-gray-600 text-gray-100 text-sm py-1.5 px-3 focus:border-purple-500 focus:ring-purple-500">
            </div>
            <div>
                <label class="block text-xs text-gray-500 mb-1">Source</label>
                <select id="detail-source" class="w-full rounded-lg bg-gray-700 border border-gray-600 text-gray-100 text-sm py-1.5 px-3 focus:border-purple-500 focus:ring-purple-500">
    ...
    <script>
        ...
        const SHORTCUT_STORAGE_KEY = 'gift_kiosk_shortcuts';
        ...
        function exportShortcuts() {
            ...
            a.download = 'gift-kiosk-shortcuts.json';
            a.click();
            ...
            reader.onload = function(e) {
                try {
                    ...
                    if (typeof imported === 'object' && !Array.isArray(imported)) {
                        ...
                        saveShortcuts();
                        applyShortcutLabels();
                        showFeedback('Shortcuts imported successfully.', 'purple');
                    } else {
        ...
        function selectCategory(btn) {
            ...
            document.querySelectorAll('.kiosk-cat-btn').forEach(b => b.classList.remove('ring-2', 'ring-purple-500'));
        ...
        document.addEventListener('keydown', function(e) {
            ...
            if (reassignMode && reassignTarget && validShortcutKeys.includes(key)) {
                ...
                reassignTarget = null;
                showFeedback(`Shortcut "${key}" assigned.`, 'purple');
                return;
            }
            ...
            .then(data => {
                if (data.found) {
                    ...
                    if (catBtn) {
                        ...
                        showFeedback(extName + ' → ' + catBtn.dataset.categoryName, 'purple');
                        submitReceipt(selectedCategoryId, catBtn.dataset.categoryName, barcode);
                    } else if (selectedCategoryId) {
                        showFeedback(extName + ' → using selected category', 'purple');
                        submitReceipt(selectedCategoryId, null, barcode);
                    } else {
                        // Unknown gift — prompt for age/gender
                        showLooseForm(barcode, extName);
                    }
                } else if (selectedCategoryId) {
                    submitReceipt(selectedCategoryId, null, barcode);
                } else {
                    // Completely unknown barcode, prompt loose donation form
                    showLooseForm(barcode, null);
                }
            })
        ...
        function showLooseForm(barcode, name) {
            pendingLooseBarcode = barcode;
            const label = document.getElementById('loose-barcode-label');
            label.textContent = name ? `Item: ${name} | Barcode: ${barcode}` : `Barcode: ${barcode} (not found)`;
            document.getElementById('loose-form').classList.remove('hidden');
            showFeedback('Unknown item — fill in gift details below.', 'yellow');
        }

        function dismissLooseForm() {
            pendingLooseBarcode = null;
            document.getElementById('loose-form').classList.add('hidden');
            input.focus();
        }

        function submitLooseDonation() {
            const age = document.getElementById('loose-age').value;
            const gender = document.getElementById('loose-gender').value;
            const barcode = pendingLooseBarcode;

            if (!selectedCategoryId) {
                showFeedback('Select a gift category first.', 'yellow');
                return;
            }
            ...
            submitReceipt(selectedCategoryId, null, barcode, null, notes);
            dismissLooseForm();
        }

        function submitReceipt(categoryId, catName, barcode, itemId, extraNotes) {
            ...
            const notes = extraNotes || document.getElementById('detail-notes')?.value || '';
        ...
        function updateDetailsBadge() {
            ...
            sidebar.style.borderLeftColor = hasDetails ? '#9333ea' : '';
