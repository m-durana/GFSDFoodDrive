<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="bg-base-100 shadow-xs sm:rounded-lg p-4 text-center">
                    <div class="text-2xl font-bold text-base-content">{{ $totals['total'] }}</div>
                    <div class="text-xs text-base-content/60 mt-1">Total Items</div>
                </div>
                <a href="{{ route('warehouse.gift-bank', ['status' => 'unassigned']) }}" class="bg-yellow-50 dark:bg-yellow-900/20 shadow-xs sm:rounded-lg p-4 text-center hover:ring-2 ring-yellow-300 transition {{ request('status') === 'unassigned' ? 'ring-2 ring-yellow-500' : '' }}">
                ...
                <a href="{{ route('warehouse.gift-bank', ['status' => 'assigned']) }}" class="bg-green-50 dark:bg-green-900/20 shadow-xs sm:rounded-lg p-4 text-center hover:ring-2 ring-green-300 transition {{ request('status') === 'assigned' ? 'ring-2 ring-green-500' : '' }}">
            ...
            <div class="bg-base-100 shadow-xs sm:rounded-lg p-4">
                <form method="GET" action="{{ route('warehouse.gift-bank') }}" class="flex flex-wrap items-end gap-4">
                    <div>
                        <label class="block text-xs text-base-content/60 mb-1">Status</label>
                        <select name="status" class="rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                    ...
                    <div>
                        ...
                        <select name="age_range" class="rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                            <option value="">All</option>
                    ...
                    <div>
                        ...
                        <select name="gender" class="rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                            <option value="">All</option>
                    ...
                    <div class="flex items-end gap-2">
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-primary text-white rounded-md hover:opacity-90 text-sm font-medium transition">Filter</button>
            ...
            <div class="bg-base-100 shadow-xs sm:rounded-lg">
                <div class="p-6">
                            ...
                            <table class="min-w-full divide-y divide-base-300">
                                <thead class="bg-base-200">
                                    <tr>
                                        ...
                                        <th class="px-3 py-2 text-left text-xs font-medium text-base-content/60 uppercase">Assigned To</th>
                                ...
                                <tbody class="divide-y divide-base-300">
                                    @foreach($items as $item)
                                        <tr>
                                            <td class="px-3 py-2 text-sm text-base-content max-w-[200px] truncate" title="{{ $item->description }}" data-sort-value="{{ $item->description }}">{{ $item->description }}</td>
                                            <td class="px-3 py-2 text-sm text-base-content" data-sort-value="{{ $item->age_range ?? '' }}">{{ $item->age_range ?? '—' }}</td>
                                            <td class="px-3 py-2 text-sm text-base-content" data-sort-value="{{ $item->gender_suitability ?? '' }}">{{ ucfirst($item->gender_suitability ?? '—') }}</td>
                                            <td class="px-3 py-2 text-sm text-base-content" data-sort-value="{{ $item->gift_type ?? '' }}">{{ $item->gift_type ?? '—' }}</td>
                                            <td class="px-3 py-2 text-sm text-base-content" data-sort-value="{{ $item->donor_name ?? '' }}">{{ $item->donor_name ?? '—' }}</td>
                                            <td class="px-3 py-2 text-sm text-right text-base-content" data-sort-value="{{ $item->quantity }}">{{ $item->quantity }}</td>
                                            <td class="px-3 py-2 text-sm" data-sort-value="{{ $item->assigned_child_id ? 'assigned' : 'available' }}">
                                                @if($item->assigned_child_id)
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded-sm text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300">Assigned</span>
                                                @else
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded-sm text-xs font-medium bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300">Available</span>
                                                @endif
                                            </td>
                    ...
                    @else
                        <p class="text-base-content/60">No items in the Gift Bank yet.</p>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div id="add-gift-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-base-100 rounded-lg shadow-xl p-6 w-full max-w-lg mx-4">
            <h3 class="text-lg font-semibold text-base-content mb-4">Add Gift to Bank</h3>
            <form method="POST" action="{{ route('warehouse.gift-bank.store') }}">
                ...
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            <label class="block text-xs text-base-content/60 mb-1">Gender Suitability</label>
                    ...
                    <div>
                        <label class="block text-xs text-base-content/60 mb-1">Notes (optional)</label>
                        <textarea name="notes" rows="2" placeholder="Any additional notes..."
                            class="w-full rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 text-sm"></textarea>
                    </div>
                </div>
    ...
    <div id="assign-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-base-100 rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
            <h3 class="text-lg font-semibold text-base-content mb-4">Assign Gift to Child</h3>
            <div class="space-y-3">
                <div>
                    <label class="block text-xs text-base-content/60 mb-1">Search by Family #</label>
                ...
                <div id="assign-results" class="space-y-1.5 max-h-[40vh] overflow-y-auto">
                    <p class="text-sm text-base-content/50 text-center py-4">Type a family number to search</p>
                </div>
            </div>
    ...
    <script>
        ...
        function searchChildren(query) {
            ...
            if (!query.trim()) {
                results.innerHTML = '<p class="text-sm text-base-content/50 text-center py-4">Type a family number to search</p>';
                return;
            }
            ...
            results.innerHTML = matches.map(c => `
                <form method="POST" action="/warehouse/gift-bank/${assignItemId}/assign/${c.id}" class="inline w-full">
                    ...
                    <button type="submit" class="w-full text-left px-3 py-2 rounded-md border border-base-300 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition text-xs">
                        <div class="flex items-center justify-between">
                            <span class="font-bold text-base-content">#${c.family_number}</span>
                            <span class="text-base-content/60">${c.gender}, age ${c.age}</span>
                        </div>
                        <div class="text-base-content/60 truncate">${c.family_name}</div>
                    </button>
                </form>
            `).join('');
        }
