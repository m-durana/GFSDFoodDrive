...
<body class="bg-slate-100 min-h-screen text-slate-900 flex items-center justify-center px-4">
    <main class="w-full max-w-sm bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
        <div class="mb-5">
        ...
        <form method="POST" action="{{ route('delivery.verifyDriverPin', $route->access_token) }}" class="space-y-4">
            ...
            <div>
                ...
                <input id="pin" name="pin" inputmode="numeric" pattern="[0-9]*" maxlength="6" autofocus
                    class="mt-1 block w-full rounded-lg border-slate-300 text-center text-2xl tracking-widest shadow-sm focus:border-red-500 focus:ring-red-500"
                    value="{{ old('pin') }}">
                @error('pin')
            ...
            <button type="submit" class="w-full rounded-lg bg-red-700 px-4 py-3 text-sm font-semibold text-white hover:bg-red-600">
                Unlock Route
            </button>
        </form>
    </main>
