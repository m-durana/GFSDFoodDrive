    ...
    <div class="flex items-start justify-between gap-2">
        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-1.5 flex-wrap">
                @if($family->route_order)
                    <span class="inline-flex items-center justify-center h-5 w-5 rounded-full text-[10px] font-bold {{ $isDone ? 'bg-green-500 text-white' : 'bg-primary text-white' }}">
                        {!! $isDone ? '&#10003;' : $family->route_order !!}
                    </span>
                @endif
                <span class="text-sm font-bold text-base-content">#{{ $family->family_number }}</span>
            ...
            <div class="mt-0.5 text-xs text-gray-600 dark:text-gray-300 space-y-0.5">
                ...
                @if($family->delivery_reason)
                    <div class="text-primary dark:text-primary">{{ $family->delivery_reason }}</div>
                @endif
        ...
        <div class="shrink-0">
            <select onchange="updateStatusAjax({{ $family->id }}, this)"
                class="rounded-sm border-base-300 dark:bg-gray-700 dark:text-gray-100 text-xs pl-2 pr-6 py-1">
