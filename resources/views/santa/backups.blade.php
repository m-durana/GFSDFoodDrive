<x-app-layout>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-base-100 shadow-xs sm:rounded-lg p-6">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h3 class="text-lg font-medium text-base-content">Backup Management</h3>
                        <p class="text-sm text-base-content/60 mt-1">
                            Automatic backups run every 4 hours. Unchanged databases are skipped.
                        </p>
                    </div>
                    <form method="POST" action="{{ route('santa.createBackup') }}">
                        @csrf
                ...
                @else
                    ...
                    @foreach($backups as $backup)
                        @if($backup['date'] !== $currentDate)
                            @php $currentDate = $backup['date']; @endphp
                            <div class="@if(!$loop->first) mt-6 @endif mb-3">
                                <h4 class="text-sm font-semibold text-base-content/80 border-b border-base-300 pb-2">
                                    {{ \Carbon\Carbon::parse($currentDate)->format('l, F j, Y') }}
                                </h4>
                            </div>
                        @endif

                        <div class="flex items-center justify-between py-3 px-4 rounded-lg mb-2 bg-base-200/50 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                            <div class="flex items-center gap-4">
                                <div class="shrink-0">
                                    <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-primary/10 dark:bg-primary/20 text-primary dark:text-primary text-sm font-bold">
                                        {{ preg_replace('/[^0-9]/', '', $backup['label']) }}
                                    </span>
                                </div>
                                <div>
                                    <div class="text-sm font-medium text-base-content">{{ $backup['label'] }}</div>
                                    <div class="text-xs text-base-content/60">
                                        {{ $backup['created_at']->format('g:i A') }}
                                        <span class="text-base-content/50">&middot; {{ $backup['created_at']->diffForHumans() }}</span>
                                    </div>
                                </div>
                            </div>

                            <div class="flex items-center gap-2">
                                <a href="{{ route('santa.downloadBackup', $backup['filename']) }}"
                                    class="inline-flex items-center px-3 py-1.5 bg-gray-200 dark:bg-gray-600 text-base-content/80 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500 text-xs font-medium transition"
                                ...
                                <button type="button"
                                    onclick="openRollbackModal('{{ $backup['filename'] }}', '{{ $backup['label'] }} ({{ $backup['created_at']->format('M j, g:i A') }})')"
            ...
            <div>
                <a href="{{ route('santa.settings') }}" class="text-sm text-base-content/70 hover:text-gray-900 dark:hover:text-gray-200 transition">
    ...
    <div id="rollback-modal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-base-100 rounded-lg shadow-xl max-w-md w-full p-6 z-10">
                <h3 class="text-lg font-semibold text-base-content mb-2">Rollback Database</h3>
                <p class="text-sm text-base-content/70 mb-1">
                    You are about to rollback to: <strong id="rollback-target" class="text-base-content"></strong>
                </p>
                ...
                <form method="POST" action="{{ route('santa.rollbackBackup') }}" id="rollback-form">
                    @csrf
                    ...
                    <div class="space-y-3 mb-6">
                        <label class="flex items-start gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-600 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <input type="radio" name="data_only" value="0" checked class="mt-0.5 text-primary focus:ring-primary">
                            <div>
                                <div class="text-sm font-medium text-base-content">Full Rollback</div>
                                <div class="text-xs text-base-content/60">Replace everything including users, settings, and all data</div>
                            </div>
                        </label>
                        <label class="flex items-start gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-600 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <input type="radio" name="data_only" value="1" class="mt-0.5 text-primary focus:ring-primary">
                            <div>
                                <div class="text-sm font-medium text-base-content">Data Only</div>
                                <div class="text-xs text-base-content/60">Restore families, children, and seasons but keep current users and settings</div>
                            </div>
                        </label>
                    </div>

                    <div class="flex gap-3">
                        <button type="button" onclick="closeRollbackModal()" class="flex-1 px-4 py-2 bg-base-300 text-base-content/80 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 text-sm font-medium transition">
                            Cancel
                        </button>
