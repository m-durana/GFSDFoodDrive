    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if(session('import_key'))
                <div class="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 text-blue-700 dark:text-blue-300 px-4 py-3 rounded-sm" id="import-progress">
                            ...
                            try {
                                ...
                                if (data.status === 'complete' || data.status === 'failed') {
                                    ...
                                    el.className = data.status === 'complete'
                                        ? 'bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 text-green-700 dark:text-green-300 px-4 py-3 rounded-sm'
                                        : 'bg-primary/5 dark:bg-primary/20 border border-primary/30 dark:border-primary text-primary dark:text-primary px-4 py-3 rounded-sm';
            ...
            @if(session('import_errors') && count(session('import_errors')) > 0)
                <div class="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 text-yellow-700 dark:text-yellow-300 px-4 py-3 rounded-sm">
            ...
            @if(!empty($legacyFiles))
                <div class="bg-base-100 shadow-xs sm:rounded-lg p-6">
                    <h3 class="text-lg font-medium text-base-content mb-2">Legacy Databases</h3>
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-sm p-3 mb-4 text-sm text-blue-700 dark:text-blue-300">
                                ...
                                <div class="flex items-center justify-between mb-2">
                                    <h4 class="font-semibold text-base-content">
                                        ...
                                        @if(in_array($year, $existingSeasons))
                                            <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded-sm text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400">Already Imported</span>
                                        @endif
                                    </h4>
                                </div>
                                <div class="space-y-2">
                                    @foreach($files as $file)
                                        <div class="flex items-center justify-between py-2 px-3 rounded-sm bg-base-200/50">
                                            <div class="flex items-center gap-3 min-w-0">
                                                <span class="shrink-0 inline-flex items-center px-2 py-0.5 rounded-sm text-xs font-mono font-medium {{ $file['ext'] === 'accdb' || $file['ext'] === 'mdb' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' }}">
                                                    .{{ $file['ext'] }}
                                                </span>
                                                <span class="text-sm text-base-content truncate" title="{{ $file['name'] }}">
                                                    {{ $file['name'] }}
                                                    @if($file['is_main'])
                                                        <span class="text-xs text-amber-600 dark:text-amber-400 font-medium ml-1">Main DB</span>
                                                    @endif
                                                </span>
                                                <span class="text-xs text-gray-400 shrink-0">{{ number_format($file['size'] / 1024) }} KB</span>
                                            </div>
                                            <form method="POST" action="{{ route('santa.seasons.importLegacy') }}" class="shrink-0 ml-3">
                                                @csrf
                                                ...
                                                <button type="submit"
                                                    ...
                                                    class="inline-flex items-center px-3 py-1.5 bg-primary text-white rounded-md hover:opacity-90 text-xs font-medium transition disabled:opacity-50 disabled:cursor-not-allowed">
                                                    Import
                                                </button>
                                            </form>
                                        </div>
            ...
            <div class="bg-base-100 shadow-xs sm:rounded-lg p-6">
                <h3 class="text-lg font-medium text-base-content mb-4">Upload File</h3>
                ...
                @if($accessDriver)
                    <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-sm p-3 mb-4">
                ...
                <form method="POST" action="{{ route('santa.seasons.previewImport') }}" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        <label for="season_year" class="block text-sm font-medium text-base-content/80">Season Year</label>
                    ...
                    <div>
                        <label for="type" class="block text-sm font-medium text-base-content/80">Data Type <span class="text-xs text-gray-400">(Excel only &mdash; Access files auto-detect)</span></label>
                    ...
                    <div>
                        <label for="file" class="block text-sm font-medium text-base-content/80">File (.xlsx, .accdb, .mdb)</label>
                        <input type="file" name="file" id="file" accept=".xlsx,.xls,.accdb,.mdb"
                            class="mt-1 block w-full text-sm text-base-content/80 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-primary/5 file:text-primary hover:file:bg-primary/10 dark:file:bg-gray-600 dark:file:text-gray-200">
                        @error('file')
                            <p class="text-primary text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            @if(count($existingSeasons) > 0)
                <div class="bg-base-100 shadow-xs sm:rounded-lg p-6">
                    <h3 class="text-lg font-medium text-base-content mb-3">Existing Seasons</h3>
                    <div class="flex flex-wrap gap-2">
                        @foreach($existingSeasons as $year)
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm bg-base-200 text-base-content/80">
                                {{ $year }}
                            </span>
                        @endforeach
                    </div>
