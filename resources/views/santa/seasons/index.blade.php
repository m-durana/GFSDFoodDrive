    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-base-100 shadow-xs sm:rounded-lg p-6">
                ...
                @if($currentStats)
                    {{-- Overview --}}
                    <h4 class="text-sm font-semibold text-base-content/60 uppercase tracking-wider mb-3">Overview</h4>
                    <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4 mb-6">
                        <div class="bg-base-200 rounded-lg p-3 text-center">
                            <div class="text-2xl font-bold text-base-content">{{ $currentStats['total_families'] }}</div>
                            <div class="text-xs text-base-content/60">Families</div>
                        </div>
                        <div class="bg-base-200 rounded-lg p-3 text-center">
                            <div class="text-2xl font-bold text-base-content">{{ $currentStats['total_children'] }}</div>
                            <div class="text-xs text-base-content/60">Children</div>
                        </div>
                        <div class="bg-base-200 rounded-lg p-3 text-center">
                            <div class="text-2xl font-bold text-base-content">{{ $currentStats['total_adults'] }}</div>
                            <div class="text-xs text-base-content/60">Adults</div>
                        </div>
                        <div class="bg-base-200 rounded-lg p-3 text-center">
                            <div class="text-2xl font-bold text-base-content">{{ $currentStats['total_family_members'] }}</div>
                            <div class="text-xs text-base-content/60">Total People</div>
                        </div>
                        <div class="bg-base-200 rounded-lg p-3 text-center">
                            <div class="text-2xl font-bold text-base-content">{{ $currentStats['avg_family_size'] }}</div>
                            <div class="text-xs text-base-content/60">Avg Family Size</div>
                        </div>
                        <div class="bg-base-200 rounded-lg p-3 text-center">
                            <div class="text-2xl font-bold text-base-content">{{ $currentStats['avg_children_per_family'] }}</div>
                            <div class="text-xs text-base-content/60">Avg Children/Family</div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                        {{-- Delivery --}}
                        <div>
                            <h4 class="text-sm font-semibold text-base-content/60 uppercase tracking-wider mb-3">Delivery</h4>
                            <div class="grid grid-cols-2 gap-3 mb-4">
                                @foreach($currentStats['families_by_delivery_status'] as $status => $count)
                                    ...
                                    @endphp
                                    <div class="rounded-lg p-3 text-center {{ $statusColors[$status] ?? 'bg-base-200' }}">
                        ...
                        {{-- Gifts --}}
                        <div>
                            <h4 class="text-sm font-semibold text-base-content/60 uppercase tracking-wider mb-3">Gifts</h4>
                            <div class="grid grid-cols-2 gap-3 mb-4">
                                <div class="bg-primary/5 dark:bg-primary/20 rounded-lg p-3 text-center">
                                    <div class="text-xl font-bold text-primary dark:text-primary">{{ $currentStats['gifts_level_0'] }}</div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        {{-- Demographics --}}
                        <div>
                            <h4 class="text-sm font-semibold text-base-content/60 uppercase tracking-wider mb-3">Demographics</h4>
                            <div class="grid grid-cols-2 gap-3 mb-4">
                            ...
                            @if(!empty($currentStats['families_by_language']))
                                <div class="bg-base-200 rounded-lg p-4">
                                    <h5 class="text-xs font-semibold text-base-content/60 uppercase mb-2">Languages</h5>
                                    <div class="space-y-2">
                                        @foreach($currentStats['families_by_language'] as $language => $count)
                                            ...
                                            @endphp
                                            <div>
                                                <div class="flex justify-between text-sm text-base-content/80 mb-1">
                                                    <span>{{ $language }}</span>
                                                    <span>{{ $count }} ({{ $pct }}%)</span>
                                                </div>
                        ...
                        <div>
                            <h4 class="text-sm font-semibold text-base-content/60 uppercase tracking-wider mb-3">Warehouse</h4>
                            ...
                            @if(!empty($currentStats['families_by_delivery_date']))
                                <h4 class="text-sm font-semibold text-base-content/60 uppercase tracking-wider mb-3">Delivery Schedule</h4>
                                <div class="bg-base-200 rounded-lg p-4">
                                    <div class="space-y-2">
            ...
            @if(count($chartYears) > 0)
                <div class="bg-base-100 shadow-xs sm:rounded-lg p-6">
                    <h3 class="text-lg font-medium text-base-content mb-4">Year-over-Year</h3>
            ...
            <!-- Past Seasons Table -->
            @if($seasons->count() > 0)
                <div class="bg-base-100 shadow-xs sm:rounded-lg overflow-hidden" x-data="sortTable()">
                    <div class="p-6">
                        <h3 class="text-lg font-medium text-base-content mb-4">Archived Seasons</h3>
                    </div>
                        ...
                        <tbody class="divide-y divide-base-300">
                            @foreach($seasons as $season)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <td class="px-6 py-4 text-sm font-medium text-base-content">{{ $season->year }}</td>
                                    <td class="px-6 py-4 text-sm text-base-content/80">{{ $season->total_families }}</td>
                                    <td class="px-6 py-4 text-sm text-base-content/80">{{ $season->total_children }}</td>
                                    <td class="px-6 py-4 text-sm text-base-content/80">{{ $season->total_family_members }}</td>
                                    <td class="px-6 py-4 text-sm text-base-content/80">{{ $season->deliveries_completed }}</td>
                                    <td class="px-6 py-4 text-sm text-base-content/60">{{ $season->archived_at?->format('M j, Y') ?? 'Imported' }}</td>
