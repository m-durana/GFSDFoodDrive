    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-base-100 shadow-xs sm:rounded-lg p-6">
                <h3 class="text-lg font-medium text-base-content mb-2">Tables Found</h3>
                    ...
                    @foreach($tables as $table)
                        <div class="border border-base-300 rounded-lg p-4">
                            <div class="flex items-center justify-between">
                                <div>
                                    <h4 class="font-medium text-base-content">{{ $table }}</h4>
                                </div>
                                <div class="flex space-x-2">
                                    <form method="POST" action="{{ route('santa.seasons.previewAccessTable') }}" class="inline">
                                        ...
                                        <button type="submit" class="px-3 py-1.5 bg-blue-600 text-white rounded-sm text-sm hover:bg-blue-500 transition">
                                            Import as Families
                                        </button>
                                    </form>
                                    <form method="POST" action="{{ route('santa.seasons.previewAccessTable') }}" class="inline">
                                        ...
                                        <button type="submit" class="px-3 py-1.5 bg-green-600 text-white rounded-sm text-sm hover:bg-green-500 transition">
                                            Import as Children
                                        </button>
                                    </form>
                                </div>
