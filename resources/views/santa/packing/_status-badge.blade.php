@php
    $badgeColors = [
        ...
        'unfulfilled' => 'bg-primary/10 text-primary dark:bg-primary/20 dark:text-primary',
    ];
@endphp
<span class="inline-flex items-center px-2 py-0.5 rounded-sm text-xs font-medium {{ $badgeColors[$status->value] ?? $badgeColors['pending'] }}">
    {{ $status->label() }}
</span>
