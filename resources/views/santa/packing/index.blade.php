<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('packing.dashboard') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-md shadow-xs transition">
                    <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" /></svg>
                    Dashboard
                </a>
                <a href="{{ route('packing.verifyStation') }}" class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-md shadow-xs transition">
                ...
                <form method="POST" action="{{ route('packing.generate') }}">
                    ...
                    <button type="submit" class="inline-flex items-center px-4 py-2 bg-primary hover:bg-primary text-white text-sm font-medium rounded-md shadow-xs transition"
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(session('error'))
                <div class="mb-4 p-4 bg-primary/5 dark:bg-primary/20 border border-primary/30 dark:border-primary/40 rounded-lg text-primary dark:text-primary text-sm">
                    {{ session('error') }}
                </div>
            @endif
                    ...
                    <a href="{{ route('packing.index', ['status' => $key]) }}"
                       class="px-4 py-2 text-sm font-medium rounded-full transition
                           {{ ($statusFilter ?? 'all') === $key
                               ...
                               : 'bg-base-200 text-base-content/80 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                        {{ $label }}
            ...
            <div class="bg-base-100 rounded-lg shadow-sm overflow-hidden"
                ...
                <!-- Batch action bar -->
                <div class="px-4 py-2 border-b border-base-300 flex items-center gap-3 bg-base-200/50"
                    ...
                    <div x-data="{ open: false }" class="relative">
                        ...
                        <div x-show="open" @click.outside="open = false"
                             class="absolute left-0 mt-1 w-36 bg-base-100 border border-gray-200 dark:border-gray-600 rounded-md shadow-lg z-10">
                            <form method="POST" action="{{ route('packing.printBatch') }}" target="_blank" x-ref="batchFormFood">
                                ...
                                <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-base-content/80 hover:bg-gray-50 dark:hover:bg-gray-700">Print Food</button>
                            </form>
                            <form method="POST" action="{{ route('packing.printBatch') }}" target="_blank">
                                ...
                                <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-base-content/80 hover:bg-gray-50 dark:hover:bg-gray-700">Print Gift</button>
                            </form>
                            <form method="POST" action="{{ route('packing.printBatch') }}" target="_blank">
                                @csrf
                                <template x-for="id in selectedIds" :key="id">
                                ...
                                <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-base-content/80 hover:bg-gray-50 dark:hover:bg-gray-700">Print Both</button>
                            </form>
                        </div>
                    </div>
                    <button @click="selectedIds = []" class="text-xs text-base-content/60 hover:underline">Clear</button>
                </div>
                    ...
                    <thead class="bg-base-200">
                        <tr>
                            <th class="px-4 py-3 w-8">
                                ...
                                <input type="checkbox" class="rounded-sm border-base-300 text-primary"
                            ...
                            <th class="px-4 py-3 text-right text-xs font-medium text-base-content/60 uppercase tracking-wider">Actions</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition"
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <div class="text-xs text-base-content/60">
                                        ...
                                        @if($list->family?->is_severe_need)
                                            <span class="ml-1 text-primary font-semibold">Severe Need</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-xs text-base-content/60">{{ $progress['packed'] }}/{{ $progress['total'] }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3 text-right space-x-2">
                                    ...
                                    <a href="{{ route('packing.print', $list) }}" class="text-sm text-base-content/70 hover:underline" target="_blank">Print</a>
