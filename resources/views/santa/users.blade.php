    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if(session('created_credentials'))
                <div id="credentials-banner" class="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 text-blue-800 dark:text-blue-200 px-4 py-3 rounded-sm">
                    <p class="font-medium mb-2">New user credentials (copy before navigating away):</p>
                    <div class="flex items-center space-x-3">
                        <code id="credentials-text" class="bg-blue-100 dark:bg-blue-800 px-3 py-1 rounded-sm text-sm font-mono">{{ session('created_credentials') }}</code>
            ...
            @if($accessRequests->count() > 0)
                <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-700 overflow-hidden shadow-xs sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <div class="space-y-3">
                            @foreach($accessRequests as $req)
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between bg-base-100 rounded-lg p-4 border border-base-300 gap-3">
                                    <div class="flex items-center gap-3">
                                        @if($req->avatar)
                                            <img src="{{ $req->avatar }}" alt="" class="w-10 h-10 rounded-full">
                                        @else
                                            <div class="w-10 h-10 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center text-base-content/60 text-sm font-bold">{{ strtoupper(substr($req->name, 0, 1)) }}</div>
                                        @endif
                                        <div>
                                            <p class="font-medium text-base-content">{{ $req->name }}</p>
                                            <p class="text-sm text-base-content/60">{{ $req->email }}</p>
                                    ...
                                    <div class="flex items-center gap-2 shrink-0">
                                        ...
                                        <form method="POST" action="{{ route('santa.denyAccessRequest', $req) }}" class="inline" onsubmit="var r=prompt('Reason (optional):');if(r!==null){this.querySelector('[name=deny_reason]').value=r;return true;}return false;">
                                            ...
                                            <button type="submit" class="inline-flex items-center px-3 py-1.5 bg-primary text-white rounded-md hover:opacity-90 text-xs font-medium transition">Deny</button>
                                        </form>
                                    </div>
                                </div>
                            @endforeach
            ...
            <div class="bg-base-100 overflow-hidden shadow-xs sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-medium text-base-content mb-4">Add New User</h3>
                    <form method="POST" action="{{ route('santa.storeUser') }}" class="space-y-4" x-data="{ newRole: '{{ old('role', 'advisor') }}' }">
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-base-content/80">Username</label>
                                <input type="text" name="username" value="{{ old('username') }}" required class="mt-1 block w-full rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                                @error('username') <p class="mt-1 text-sm text-primary">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-base-content/80">First Name</label>
                                <input type="text" name="first_name" value="{{ old('first_name') }}" required class="mt-1 block w-full rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-base-content/80">Last Name</label>
                                <input type="text" name="last_name" value="{{ old('last_name') }}" required class="mt-1 block w-full rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-base-content/80">Role</label>
                                <select name="role" x-model="newRole" required class="mt-1 block w-full rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                            ...
                            <div x-show="newRole === 'advisor' || newRole === 'coordinator'">
                                <label class="block text-sm font-medium text-base-content/80">School</label>
                                <select name="school_source" class="mt-1 block w-full rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                            ...
                            <div x-show="newRole === 'coordinator' || newRole === 'santa'">
                                <label class="block text-sm font-medium text-base-content/80">Position</label>
                                <select name="position" class="mt-1 block w-full rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                                    <option value="">-- None --</option>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-base-content/80">Password</label>
                                <div class="mt-1 flex rounded-md shadow-xs">
                                    <input type="text" name="password" id="new-password" required class="block w-full rounded-l-md border-base-300 dark:bg-gray-700 dark:text-gray-100 text-sm">
                                    <button type="button" onclick="generatePassword()" class="inline-flex items-center px-3 rounded-r-md border border-l-0 border-base-300 bg-gray-50 dark:bg-gray-600 text-gray-500 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-500 text-xs" title="Generate">
                        ...
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-primary text-white rounded-md hover:opacity-90 text-sm font-medium transition">Add User</button>
                    </form>
                </div>
            </div>
            ...
            <div class="bg-base-100 overflow-hidden shadow-xs sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-medium text-base-content mb-4">Existing Users ({{ $users->count() }})</h3>
                    ...
                    <form method="POST" action="{{ route('santa.bulkUpdateUsers') }}" id="bulk-form">
                        ...
                        <div class="overflow-x-auto" x-data="sortTable()">
                            <table class="min-w-full divide-y divide-base-300 text-sm">
                                <thead class="bg-base-200">
                                    <tr>
                                        <th class="px-3 py-3 text-left text-xs font-medium text-base-content/60 uppercase">Avatar</th>
                                        ...
                                        <th class="px-3 py-3 text-left text-xs font-medium text-base-content/60 uppercase">New Password</th>
                                        <th class="px-3 py-3 text-left text-xs font-medium text-base-content/60 uppercase">Flags</th>
                                        <th class="px-3 py-3 text-left text-xs font-medium text-base-content/60 uppercase">Actions</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-base-300" x-data>
                                    @foreach($users as $u)
                                        @php $isCurrentUser = $u->id === auth()->id(); @endphp
                                        <tr class="{{ $u->permission === 0 ? 'bg-base-200/50 opacity-60' : '' }}" x-data="{ role: '{{ match($u->permission) { 0 => 'inactive', 7 => 'advisor', 8 => 'coordinator', 9 => 'santa', default => 'advisor' } }}' }">
                                            <!-- Avatar -->
                                            ...
                                            <td class="px-3 py-2" data-sort-value="{{ $u->username }}">
                                                <span class="font-medium text-base-content truncate block max-w-[120px]" title="{{ $u->username }}">{{ $u->username }}</span>
                                            </td>
                                            <!-- First Name -->
                                            <td class="px-3 py-2" data-sort-value="{{ $u->first_name }}">
                                                <input type="text" name="users[{{ $u->id }}][first_name]" value="{{ $u->first_name }}"
                                                    class="w-full min-w-[80px] rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                                            </td>
                                            <!-- Last Name -->
                                            ...
                                            <td class="px-3 py-2" data-sort-value="{{ match($u->permission) { 0 => 'inactive', 7 => 'advisor', 8 => 'coordinator', 9 => 'santa', default => 'advisor' } }}">
                                                <select name="users[{{ $u->id }}][role]" x-model="role"
                                                    class="w-full min-w-[100px] rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                                            ...
                                            <td class="px-3 py-2" data-sort-value="{{ $u->position ?? '' }}">
                                                <select name="users[{{ $u->id }}][position]" :disabled="role === 'advisor' || role === 'inactive'"
                                                    class="w-full min-w-[100px] rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm disabled:opacity-40 disabled:cursor-not-allowed">
                                                    <option value="">--</option>
                                                    @foreach($positions as $pos)
                                            ...
                                            <td class="px-3 py-2" x-data="{ generatedPw: '' }">
                                                ...
                                                <div class="flex items-center gap-1">
                                                    ...
                                                    <button type="button" x-show="generatedPw" @click="generatedPw = ''" class="text-gray-400 hover:text-primary transition" title="Clear password">
                                            ...
                                            <td class="px-3 py-2">
                                                <div class="flex flex-col gap-1 text-xs">
                                                    <label class="inline-flex items-center gap-1 whitespace-nowrap" :class="role !== 'coordinator' && role !== 'santa' ? 'opacity-40' : ''" title="Force show on homepage (overrides user preference)">
                                                        ...
                                                        <span class="text-base-content/80">Homepage</span>
                                                    </label>
                                            ...
                                            <td class="px-3 py-2">
                                                @unless($isCurrentUser)
                                                    <form method="POST" action="{{ route('santa.deleteUser', $u) }}" class="inline" onsubmit="return confirm('Delete user {{ $u->username }}? This cannot be undone.')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-primary hover:text-primary dark:hover:text-primary transition" title="Delete user">
                        ...
                        <div class="sticky bottom-0 bg-base-100 border-t border-base-300 pt-4 mt-4 flex justify-end">
                            <button type="submit" class="inline-flex items-center px-6 py-2.5 bg-primary text-white rounded-md hover:opacity-90 text-sm font-medium transition shadow-xs">
    ...
    <div x-data="{ toasts: [] }" @show-toast.window="toasts.push($event.detail); setTimeout(() => toasts.shift(), 3000)"
        ...
        <template x-for="(toast, i) in toasts" :key="i">
            <div x-transition class="px-4 py-3 rounded-lg shadow-lg text-sm font-medium text-white max-w-sm"
                 :class="toast.type === 'error' ? 'bg-primary' : 'bg-green-600'">
