        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                <div class="bg-base-100 shadow-xs sm:rounded-lg p-4 text-center">
                    <div class="text-2xl font-bold text-base-content">{{ $counts['total'] }}</div>
                    <div class="text-xs text-base-content/60 mt-1">Total Children</div>
                </div>
                <a href="{{ route('santa.gifts', ['level' => 0]) }}" class="bg-primary/5 dark:bg-primary/20 shadow-xs sm:rounded-lg p-4 text-center hover:ring-2 ring-primary/40 transition {{ request('level') === '0' ? 'ring-2 ring-primary' : '' }}">
                    <div class="text-2xl font-bold text-primary dark:text-primary">{{ $counts['no_gifts'] }}</div>
                    <div class="text-xs text-primary dark:text-primary mt-1">No Gifts</div>
                </a>
                <a href="{{ route('santa.gifts', ['level' => 1]) }}" class="bg-yellow-50 dark:bg-yellow-900/20 shadow-xs sm:rounded-lg p-4 text-center hover:ring-2 ring-yellow-300 transition {{ in_array(request('level'), ['1', '2']) ? 'ring-2 ring-yellow-500' : '' }}">
                ...
                <a href="{{ route('santa.gifts', ['level' => 3]) }}" class="bg-green-50 dark:bg-green-900/20 shadow-xs sm:rounded-lg p-4 text-center hover:ring-2 ring-green-300 transition {{ request('level') === '3' ? 'ring-2 ring-green-500' : '' }}">
                ...
                <a href="{{ route('santa.gifts', ['merged' => '0']) }}" class="bg-blue-50 dark:bg-blue-900/20 shadow-xs sm:rounded-lg p-4 text-center hover:ring-2 ring-blue-300 transition {{ request('merged') === '0' ? 'ring-2 ring-blue-500' : '' }}">
            ...
            <div class="bg-base-100 shadow-xs sm:rounded-lg p-4">
                <form method="GET" action="{{ route('santa.gifts') }}" class="flex flex-wrap items-end gap-4">
                    <div>
                        <label class="block text-xs text-base-content/60 mb-1">Gift Level</label>
                        <select name="level" class="rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                    ...
                    <div>
                        <label class="block text-xs text-base-content/60 mb-1">Tag Status</label>
                        <select name="merged" class="rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                    ...
                    <div>
                        <label class="block text-xs text-base-content/60 mb-1">Adopted</label>
                        <select name="adopted" class="rounded-md border-base-300 dark:bg-gray-700 dark:text-gray-100 shadow-xs text-sm">
                            <option value="">All</option>
            ...
            <div class="bg-base-100 shadow-xs sm:rounded-lg">
                <div class="p-6">
                                ...
                                <tbody class="divide-y divide-base-300">
                                    @foreach($children as $child)
                                        <tr class="{{ $child->family->family_done ? 'bg-green-50/50 dark:bg-green-900/10' : '' }}">
                                            ...
                                            <td class="px-3 py-2 text-sm text-base-content">{{ $child->gender }}</td>
                                            <td class="px-3 py-2 text-sm text-base-content">{{ $child->age }}</td>
                                            <td class="px-3 py-2 text-sm text-base-content">{{ $child->school ?? '—' }}</td>
                                            <td class="px-3 py-2 text-sm">
                                            ...
                                            <td class="px-3 py-2 text-sm text-base-content max-w-[200px]" x-data="{ expanded: false }">
                                            ...
                                            <td class="px-3 py-2 text-sm text-base-content">{{ $child->where_is_tag ?? '—' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    @else
                        <p class="text-base-content/60">No children match the selected filters.</p>
                    @endif
                </div>
            </div>
